  ...
  return {
    ...
    renderTick: (_, __, done, total) => {
      ...
      process.stdout.write(
        `\r  Рендеринг: ${done}/${total}  ${"█".repeat(Math.round(pct / 5))}${"░".repeat(20 - Math.round(pct / 5))}  ${pct}%`,
      );
      if (done === total) process.stdout.write("\n");
    },
    assemblyProgress: (pct) => {
      process.stdout.write(
        `\r  Сборка: ${"█".repeat(Math.round(pct / 5))}${"░".repeat(20 - Math.round(pct / 5))}  ${pct.toFixed(0)}%`,
      );
...
async function resolveQuality(
  qualityLevel: string,
  filePaths: string[],
): Promise<RenderQuality> {
  const level =
    (qualityLevel as keyof typeof QUALITY_TEMPLATES) in QUALITY_TEMPLATES
  ...
  const bitrate = template.bitrateAuto
    ...
    : template.bitrate;
  if (template.bitrateAuto)
    console.log(
      `  Auto bitrate: ${bitrate} Mbps (медиана по ${filePaths.length} файлам)`,
    );
...
async function checkOllamaAvailable(model: string): Promise<void> {
  ...
  const names: string[] = (
    typeof data === "object" && data !== null && "models" in data
      ? (((data as Record<string, unknown>).models as Array<{
          name: string;
        }>) ?? [])
      : []
  ).map((m) => m.name);
...
async function runMainPipeline(
  ...
  try {
    ...
    if (allFiles.length === 0)
      throw new Error(`Видеофайлы не найдены в: ${config.input}`);
...
async function runEditPipeline(
  ...
  try {
    ...
    cb.info(
      `${qualityLevel} · ${quality.bitrate} Mbps · ${quality.x264preset}${quality.scale ? ` · ${quality.scale}` : ""}`,
    );
...
async function runIndexOnly(): Promise<void> {
  ...
  const input =
    (inputIdx !== -1 ? args[inputIdx + 1] : undefined) ??
    loadSavedConfig().input ??
    "";
  const model =
    (modelIdx !== -1 ? args[modelIdx + 1] : undefined) ??
    loadSavedConfig().model ??
    "llava:13b";
  ...
  const TEMP_DIR = path.join(
    os.tmpdir(),
    `automounter_idx_${Date.now().toString(36)}`,
  );
  try {
    console.log(`\n  Индексация: ${input}\n  Модель:     ${model}\n`);
...
async function main(): Promise<void> {
  ...
  if (result.mode === "create") {
    ...
    await showPipelineUI(result.config, (cb) =>
      runMainPipeline(result.config, cb),
    );
    ...
    await showPipelineUI(
      ...
      async (cb) => {
        ...
        try {
          const videos = await indexMediaFolder(
            result.input,
            TEMP_DIR,
            result.model,
            undefined,
