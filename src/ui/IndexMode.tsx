...
export const IndexMode = ({
  ...
  const [step, setStep] = useState<"input" | "model" | "reindex" | "confirm">(
    "input",
  );
      ...
      case "reindex":
        ...
        return (
          <box style={{ flexDirection: "column", flexGrow: 1 }}>
            ...
            <select
              options={reindexItems}
              onSelect={(idx) => {
                setReindex(reindexItems[idx].value as boolean);
                setStep("confirm");
              }}
              focused={true}
              {...selectStyles}
    ...
    >
      ...
      >
        <box style={{ flexDirection: "row" }}>
          <box style={{ width: 12 }}>
            <text style={{ fg: THEME.dim }}>Path: </text>
          </box>
          <text style={{ fg: THEME.highlight }}>{input}</text>
        </box>
        <box style={{ flexDirection: "row" }}>
          <box style={{ width: 12 }}>
            <text style={{ fg: THEME.dim }}>Model: </text>
          </box>
          <text style={{ fg: THEME.highlight }}>{model}</text>
        </box>
      </box>
      <box
        style={{
