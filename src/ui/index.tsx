...
function cleanupTerminal() {
  ...
  process.stdout.write(
    "\x1bc\x1b[?1000l\x1b[?1002l\x1b[?1003l\x1b[?1006l\x1b[?1015l\x1b[?25h\x1b[?1049l\x1b[0m",
  );
...
const MainMenu = ({ hasSession, onSelect }: MenuProps) => {
  const items = [
    ...
    ...(hasSession
      ? [
          {
            name: "  ✎  Правка последнего ролика",
            description: "Изменить LUT или качество",
            value: "edit",
          },
        ]
      : []),
    {
      name: "  ⟳  Переиндексация папки",
      description: "Обновить базу данных видео",
      value: "index",
    },
  ];
  return (
    ...
    >
      <box style={{ justifyContent: "space-between", marginBottom: 1 }}>
        ...
        <text style={{ fg: THEME.dim }}>CLI Edition </text>
      </box>
      <text style={{ fg: THEME.border, marginBottom: 1 }}>
        {"─".repeat(42)}
      </text>
      <box style={{ flexGrow: 1, padding: 1 }}>
        <select
          options={items}
          onSelect={(index) => onSelect(items[index].value as any)}
          focused={true}
...
const App = ({ saved, session, cwd, onDone }: AppProps) => {
  const [mode, setMode] = useState<"menu" | "create" | "edit" | "index">(
    "menu",
  );
  return (
    <box
    ...
    >
      {mode === "menu" ? (
        <MainMenu hasSession={session !== null} onSelect={setMode} />
      ) : null}
      {mode === "create" ? (
        <CreateMode
          saved={saved}
    ...
    root.render(
      <PipelineView
        ...
        onError={(msg) => {
          root.unmount();
          cleanupTerminal();
          setTimeout(() => reject(new Error(msg)), 50);
        }}
      />,
    );
  });
...
export async function showInkUI(
  saved: Partial<Config>,
  cwd: string,
): Promise<AppResult> {
  const renderer = await createCliRenderer({ exitOnCtrlC: true });
  ...
  return new Promise<AppResult>((resolve) => {
    root.render(
      <App
        saved={saved}
        session={(saved.lastSession as RenderSession | undefined) ?? null}
        cwd={cwd}
        onDone={(result) => {
          root.unmount();
          cleanupTerminal();
