...
export async function promptSetup(saved: Partial<Config>): Promise<Config> {
  ...
  const durStr = await optionalField(
    rl,
    "Duration (sec)",
    saved.duration ?? 60,
  );
  const bitrateStr = await optionalField(
    rl,
    "Bitrate (Mbps)",
    saved.bitrate || "auto",
  );
  const lut = await optionalField(
    rl,
    "LUT file",
    saved.lut ?? "cinematic.cube",
  );
  const output = await optionalField(
    rl,
    "Output file",
    saved.output ?? "final_edit.mp4",
  );
  const model = await optionalField(
    rl,
    "Ollama model",
    saved.model ?? "llava:13b",
  );
  ...
  const promptPreview =
    config.prompt.length > 72
      ? config.prompt.slice(0, 72) + "…"
      : config.prompt;
  console.log(`  Prompt:   "${promptPreview}"`);
  console.log(
    `  Duration: ${config.duration}s  |  Bitrate: ${config.bitrate > 0 ? config.bitrate + " Mbps" : "auto"}`,
  );
