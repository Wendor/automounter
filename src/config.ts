...
function parseArgs(argv: string[]): Partial<Config> {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === "--bitrate" && next) {
      result.bitrate = parseInt(next, 10);
      i++;
    }
    if (arg === "--quality" && next) {
...
export function loadConfig(): Config {
  ...
  if (!merged.input)
    throw new Error("--input is required (path to video folder)");
  if (!merged.audio)
    throw new Error("--audio is required (path to audio file)");
  if (!merged.prompt)
    throw new Error("--prompt is required (editing instruction)");
