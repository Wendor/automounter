...
async function getFilterDirectivesFromAI(prompt: string, allTags: string[], modelName: string): Promise<FilterDirectives> {
...
function applyAiFiltering(videos: VideoInfo[], directives: FilterDirectives): VideoInfo[] {
    const filtered = videos.filter(v => {
        const text = (v.tags?.join(' ') + ' ' + (v.description || '')).toLowerCase();
        return !directives.excludeKeywords.some(k => text.includes(k.toLowerCase()));
    });
...
function analyzePromptIntent(prompt: string): PromptIntent {
    ...
    const lt = ['water', 'forest', 'urban', 'mountains', 'road', 'coast', 'city', 'nature', 'snow'];
...
function calculateClipRelevance(video: VideoInfo, intent: PromptIntent, directives: FilterDirectives, energy: number, usedCount: number): number {
    let score = 0;
    const text = (video.tags?.join(' ') + ' ' + (video.description || '')).toLowerCase();
    if (directives.excludeKeywords.some(k => text.includes(k.toLowerCase()))) score -= 15.0;
    if (usedCount > 0) score -= 10.0 * usedCount;
    for (const kw of directives.priorityKeywords) if (text.includes(kw.toLowerCase())) score += 4.0;
    for (const kw of intent.keywords) if (text.includes(kw)) score += 3.0;
    if (intent.style === 'fast-paced') score += (video.motionIntensity ?? 0.5) * 2.0;
    else if (intent.style === 'calm') score += (1 - (video.motionIntensity ?? 0.5)) * 2.0;
    score += (video.aestheticScore ?? 5) / 2;
    score += (1 - Math.abs((video.motionIntensity ?? 0.5) - energy)) * 1.5;
    return score;
}

function findNearestPeak(time: number, peaks: number[], maxDelta: number = 0.1): number {
    let nearest = time;
...
function buildAutoInstructions(
    ...
    while (beatsUsed < beats.length && accumulatedSecs < targetDurationSeconds) {
        ...
        for (const candidate of pool) {
            ...
            let score = calculateClipRelevance(candidate, intent, directives, currentEnergy, uses);
...
export async function preFilterFiles(
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (parsed.dateStart) filtered = filtered.filter(f => new Date(f.date) >= new Date(parsed.dateStart));
            if (parsed.dateEnd) filtered = filtered.filter(f => new Date(f.date) <= new Date(parsed.dateEnd));
...
export async function createDirectorPlan(
    ...
    const directives = await getFilterDirectivesFromAI(userPrompt, allTags, textModelName);
    const filteredVideos = applyAiFiltering(videos.filter(v => v.validZones.length > 0), directives);
    const autoPlan = buildAutoInstructions(filteredVideos, audioAnalysis, directives, targetDurationSeconds, userPrompt);
    ...
    for (const inst of autoPlan) {
        ...
        if (!video) continue;
        const activeZone = video.bestSegments?.[0] || video.validZones.sort((a,b) => (b.end-a.start)-(a.end-a.start))[0];
        ...
        let rawDur = Math.max(0.5, Math.min(targetDur * slowMo, activeZone.end - activeZone.start));
        let startTime = usageCount === 1 ? Math.max(activeZone.start, activeZone.end - rawDur - 1.0) : activeZone.start + Math.random() * Math.max(0, (activeZone.end - rawDur) - activeZone.start);
        ...
        segments.push({
            ...
            isLast: false,
            effect: (segments.length === 0) ? 'fadeIn' : ((video.motionIntensity ?? 0.5) < 0.4 ? (Math.random() > 0.5 ? 'zoomIn' : 'zoomOut') : 'none'),
    ...
    if (segments.length > 0) { segments[segments.length-1]!.isLast = true; segments[segments.length-1]!.effect = 'fadeOut'; }
