...
function buildVideoFilter(
    ...
    filters.push('scale=1920:1080:force_original_aspect_ratio=increase,crop=1920:1080,setsar=1');
    
    // Плавный зум, растянутый на всю длительность сегмента
    const effect: SegmentEffect = segment.effect;
    if (effect === 'zoomIn') {
        filters.push(`scale='iw*(1+0.08*t/${segment.targetDuration})':-1,crop=1920:1080`);
    } else if (effect === 'zoomOut') {
        filters.push(`scale='iw*(1.08-0.08*t/${segment.targetDuration})':-1,crop=1920:1080`);
    }
    ...
    switch (effect) {
        case 'fadeIn':   filters.push(`fade=t=in:st=0:d=${videoFadeDuration}`); break;
        case 'fadeOut':  filters.push(`fade=t=out:st=${fadeOutStart}:d=${videoFadeDuration}`); break;
        case 'flashIn':  filters.push(`fade=t=in:st=0:d=${videoFadeDuration}:color=white`); break;
        case 'flashOut': filters.push(`fade=t=out:st=${fadeOutStart}:d=${videoFadeDuration}:color=white`); break;
    }
...
function renderSegment(
    ...
    if (fs.existsSync(segment.outputFile)) { onProgress?.(index, 100); return Promise.resolve(); }
    ...
    return new Promise((resolve, reject) => {
        ffmpeg(segment.sourceFile)
            ...
            .on('progress', (p: any) => { if (p.timemark) onProgress?.(index, Math.min(99, (timemarkToSeconds(p.timemark) / segment.rawDuration) * 100)); })
...
async function renderWithConcurrency(
    segments: VideoSegment[], codec: string, targetFps: number, hasLut: boolean, lutFile: string, videoFadeDuration: number, quality: RenderQuality, concurrency: number, onProgress?: RenderProgressCallback
): Promise<void> {
    let i = 0;
...
export async function renderSegments(
    segments: VideoSegment[], tempDir: string, lutFile: string, videoFadeDuration: number, totalDuration: number, quality: RenderQuality, onProgress?: RenderProgressCallback
): Promise<SliceResult> {
...
function buildXfadeFiltergraph(segments: SegmentConcatInfo[]): { filterLines: string[], duration: number } {
    const lines: string[] = [];
    let cursor = 0;
...
async function mergeChunk(segments: SegmentConcatInfo[], outputFile: string, targetFps: number, quality: RenderQuality): Promise<number> {
    if (segments.length === 1) {
        return new Promise((res, rej) => {
            ffmpeg(segments[0]!.file).outputOptions(['-c:v', 'copy', '-an']).on('end', () => res(segments[0]!.targetDuration)).on('error', rej).save(outputFile);
        });
    }
    ...
    return new Promise((resolve, reject) => {
        ...
        cmd.complexFilter(filterLines).outputOptions(['-map', '[vout]', '-c:v', 'libx264', '-preset', 'ultrafast', '-r', String(targetFps), '-pix_fmt', 'yuv420p', '-max_muxing_queue_size', '1024', '-an']).on('end', () => resolve(duration)).on('error', (err) => { console.error(`Chunk Merge Error: ${err.message}`); reject(err); }).save(outputFile);
    });
}

export async function concatenateAndAddMusic(
    segments: SegmentConcatInfo[], audioFile: string, totalDuration: number, audioFadeDuration: number, outputFile: string, tempDir: string, targetFps: number, quality: RenderQuality, onProgress?: (pct: number) => void
): Promise<void> {
    const CHUNK_SIZE = 4;
    const chunks: string[] = [];
    ...
    for (let i = 0; i < segments.length; i += CHUNK_SIZE) {
        ...
        console.log(`  -> Assembling chunk ${chunks.length + 1}/${Math.ceil(segments.length / CHUNK_SIZE)}...`);
    ...
    return new Promise((resolve, reject) => {
        ffmpeg().input(listFile).inputOptions(['-f', 'concat', '-safe', '0']).input(audioFile).complexFilter([`[1:a]afade=t=out:st=${audioFadeStart}:d=${audioFadeDuration}[aout]`]).outputOptions(['-map', '0:v:0', '-map', '[aout]', '-c:v', 'libx264', '-b:v', `${quality.bitrate || 25}M`, '-preset', quality.x264preset, '-r', String(targetFps), '-c:a', 'aac', '-b:a', '320k', '-t', String(finalVideoDur), '-pix_fmt', 'yuv420p']).on('progress', (p: any) => { if (p.timemark) onProgress?.(50 + Math.min(49, (timemarkToSeconds(p.timemark) / finalVideoDur) * 50)); }).on('end', () => resolve()).on('error', reject).save(outputFile);
    });
}